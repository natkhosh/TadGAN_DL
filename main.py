...
def critic_x_iteration(sample):
    ...
    # STEP 1 : Изменение размера x
    x = sample['signal'].view(1, batch_size, signal_shape)

    # STEP 2 : Вычисление оценок аномалий с помощью выходных данных Critic X
    valid_x = critic_x(x)
    valid_x = torch.squeeze(valid_x)
    critic_score_valid_x = torch.mean(torch.ones(valid_x.shape) * valid_x)          # Wasserstein Loss

    # STEP 3 : Вычисление оценок аномалий с помощью выходных данных Critic X
    ...
    # STEP 4 : Вычислить gradient penalty
    ...
    v_ix.mean().backward()
    gradients = ix.grad
    gp_loss = torch.sqrt(torch.sum(torch.square(gradients).view(-1)))               # Gradient Penalty Loss
    ...
    # STEP 6 : вычисления градиента
    loss.backward()
    # STEP 7 : обновление весов Сх
    optim_cx.step()
...
def critic_z_iteration(sample):
    ...
    # STEP 1 : Изменение размера z
    x = sample['signal'].view(1, batch_size, signal_shape)
    z = encoder(x)

    # STEP 2 : Вычисление оценок аномалий с помощью выходных данных Critic Z
    valid_z = critic_z(z)
...
def train(n_epochs=2000):
    """
    n_critics = количество итераций критика за эпоху
    n_epoch = количество итераций над данными
    """
    ...
    # запускаем обучение в цикле по эпохам
    for epoch in range(n_epochs):
        ...
        #  обучаем дискринаторы Cx и Cz (различающие сети) n_critics = 5 раз
        for i in range(n_critics):
            cx_loss = list()
            cz_loss = list()

            for batch, sample in enumerate(train_loader):
                loss = critic_x_iteration(sample)                       # вычисляем Wasserstein loss для critic X
                cx_loss.append(loss)

                loss = critic_z_iteration(sample)                       # вычисляем Wasserstein loss для critic Z
                cz_loss.append(loss)

            # вычисляем среднее значение CriticX loss и CriticZ loss по всем семплам
        ...
        # Обучаем генерирующую сеть E и G с адаптированным циклом потерь согласованности,
        # минимизируя L2 норму между исходной и реконструированной выборками
        for batch, sample in enumerate(train_loader):
            # вычислить потерю согласованности адаптированного цикла, минимизируя норму L2 на кодировщике
            enc_loss = encoder_iteration(sample)
            # вычислить потерю согласованности адаптированного цикла, минимизируя норму L2 в декодере
            dec_loss = decoder_iteration(sample)
            # добавляем потерю кодировщика в список, где индекс соответствует выборке
            encoder_loss.append(enc_loss)
            decoder_loss.append(dec_loss)

        # предварительно обрабатываем данные для печати
...
if __name__ == "__main__":
    ...
    dataset = pd.read_csv('nyc_taxi_new.csv')
