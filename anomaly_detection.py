...
# Другие показатели ошибок - разница в точках, разница в площади.
...
def unroll_signal(x):
    """
    Берем медианное значение из набора/коллекции в качестве окончательного реконструированного значения.
    В предварительных экспериментах было обнаружено, что использование медианы обеспечивает лучшую производительность,
    чем использование среднего.
    """
...
def prune_false_positive(is_anomaly, anomaly_score, change_threshold):
    """
    Функция отсечения ложных срабатываний
    """
...
def detect_anomaly(anomaly_score):
    """
    Функция обнаружения аномалии.
    Определяем обучающую выборку, введя скользящее окно.
    Вводим скользящее окно с размером окна window_size и размером шага step_size.
    Исходный временной ряд делится на N подпоследовательностей
    """
    # Используем скользящие окна для вычисления пороговых значений и эмпирически устанавливаем размер окна как Т/3
    # и размер шага как Т/(3*10)б,  Т = anomaly_score
    ...
    for i in range(0, len(anomaly_score) - window_size, step_size):
        ...
        # Затем идентифицируем те точки, оценка аномалии которых превышает пороговое значение, как аномальные.
...
def find_scores(y_true, y_predict):
    """
    Функция расчета оценок
    """
    EPS = 1e-7  # избежать деления на 0
    tp = tn = fp = fn = 0
